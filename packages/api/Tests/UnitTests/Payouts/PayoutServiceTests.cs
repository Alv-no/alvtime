...
namespace Tests.UnitTests.Payouts
{
    public class PayoutServiceTests
    {
        ...
        public void GetRegisteredPayouts_Registered10Hours_10HoursRegistered()
        {
            ...
            var timeEntry1 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 13), 17.5M, 1.0M, out int taskId1); //Monday
        ...
        public void RegisterPayout_CalculationCorrectForBeforeAndAfterCompRate()
        {
            ...
            var timeEntry2 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 14), 17.5M, 0.5M, out _); //Tuesday
        ...
        {
            var timeRegistrationService = CreateTimeRegistrationService();
            var timeEntry1 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 13), 8.5M, 1.5M, out _); //Monday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry1.Date, Value = timeEntry1.Value, TaskId = timeEntry1.TaskId } });

            var timeEntry2 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 14), 12.5M, 0.5M, out _); //Tuesday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry2.Date, Value = timeEntry2.Value, TaskId = timeEntry2.TaskId } });
            
            var timeEntry3 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 15), 9.0M, 1.0M, out _); //Wednesday
            ...
            var timeEntry4 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 16), 9.5M, 1.5M, out _); //Thursday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry4.Date, Value = timeEntry4.Value, TaskId = timeEntry4.TaskId } });
        ...
        {
            var timeRegistrationService = CreateTimeRegistrationService();
            var timeEntry1 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 13), 11.5M, 1.5M, out _); //Monday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry1.Date, Value = timeEntry1.Value, TaskId = timeEntry1.TaskId } });
        ...
        {
            var timeRegistrationService = CreateTimeRegistrationService();
            var timeEntry1 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 13), 7.5M, 1.5M, out _); //Monday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry1.Date, Value = timeEntry1.Value, TaskId = timeEntry1.TaskId } });
            
            var timeEntry2 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 13), 3M, 0.5M, out _); //Monday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry2.Date, Value = timeEntry2.Value, TaskId = timeEntry2.TaskId } });
            
            var timeEntry3 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 13), 1.5M, 1.0M, out _); //Monday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry3.Date, Value = timeEntry3.Value, TaskId = timeEntry3.TaskId } });

            var payoutService = CreatePayoutService(timeRegistrationService);
            var result = payoutService.RegisterPayout(new GenericHourEntry
            {
                Date = new DateTime(2021, 12, 14), //Thursday
                Hours = 4
            });

            var timeEntry4 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 15), 7.5M, 1.5M, out _); //Wednesday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry4.Date, Value = timeEntry4.Value, TaskId = timeEntry4.TaskId } });
            
            var timeEntry5 = CreateTimeEntryWithCompensationRate(new DateTime(2021, 12, 15), 1.5M, 0.5M, out _); //Wednesday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry5.Date, Value = timeEntry5.Value, TaskId = timeEntry5.TaskId } });
            
            var overtimeEntriesAtPayoutDate = timeRegistrationService.GetAvailableOvertimeHours();
            ...
            var overtimeEntriesAfterPayoutDate = timeRegistrationService.GetAvailableOvertimeHours();
        ...
        public void CancelPayout_PayoutIsRegisteredInSameMonth_PayoutIsCanceled()
        {
            ...
            var timeRegistrationService = CreateTimeRegistrationService();
            var timeEntry1 = CreateTimeEntryWithCompensationRate(new DateTime(currentYear, currentMonth, 02), 17.5M, 1.0M, out _); //Monday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry1.Date, Value = timeEntry1.Value, TaskId = timeEntry1.TaskId } });
        ...
        public void CancelPayout_PayoutIsRegisteredPreviousMonth_PayoutIsLocked()
        {
            ...
            var timeRegistrationService = CreateTimeRegistrationService();
            var timeEntry1 = CreateTimeEntryWithCompensationRate(new DateTime(yearToTest, previousMonth, 02), 17.5M, 1.0M, out _); //Monday
            timeRegistrationService.UpsertTimeEntry(new List<CreateTimeEntryDto>{ new() {Date = timeEntry1.Date, Value = timeEntry1.Value, TaskId = timeEntry1.TaskId } });

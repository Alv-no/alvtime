...
public class OvertimeTests
{
    ...
    public async System.Threading.Tasks.Task GetTimeEntries_UpdateOvertimeFails_NoHoursRegistered()
    {
        ...
        var timeRegistrationService = new TimeRegistrationService(_options, _userContextMock.Object,
            CreateTaskUtils(), mockRepo.Object, new DbContextScope(sqliteContext), new PayoutStorage(sqliteContext, new DateAlvTime()), new UserService(new UserRepository(sqliteContext), new TimeRegistrationStorage(sqliteContext)));
        var dateToTest = new DateTime(2021, 12, 13); //Monday
    ...
    public async System.Threading.Tasks.Task GetOvertime_StartWithNegativeValue_GivesCorrectPayout()
    {
        ...
        Assert.True(registeredPayoutResult.IsSuccess);
        var registeredPayout = registeredPayoutResult.Value;
        Assert.Equal(5.5M, registeredPayout.HoursAfterCompensation);
    }
    ...
    public async System.Threading.Tasks.Task GetOvertime_RegisterImposedOvertimeAndOrderPayout_ImposedOvertimeIsPutOnTop()
    {
        ...
        Assert.True(registeredPayoutResult.IsSuccess);
        var registeredPayout = registeredPayoutResult.Value;
        Assert.Equal(4.0M, registeredPayout.HoursAfterCompensation);
    }
    ...
    public async System.Threading.Tasks.Task GetOvertime_RegisterImposedOvertimeWithPayout_ImposedOvertimeIsPutOnTop2()
    {
        ...
        Assert.True(registeredPayoutResult.IsSuccess);
        var registeredPayout = registeredPayoutResult.Value;
        Assert.Equal(2.75M, registeredPayout.HoursAfterCompensation);
    }
    ...
    public async System.Threading.Tasks.Task UpdateOvertime_HasFutureFlex_GetsNegativeBalance()
    {
        ...
        var timeEntryResult = await _timeRegistrationService.UpsertTimeEntry(

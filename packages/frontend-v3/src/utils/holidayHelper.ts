...
const createNorwegianHolidaysForYear = (yearNumber: number) => {
	...
	const easterDateOffset = (daysToOffset: number, easter: Date) => {
  	...
  	const norwegianHolidays = [
		...
		{ date: easterDateOffset(-7, easter), description: "Palmesøndag" },
		{ date: easterDateOffset(-3, easter), description: "Skjærtorsdag" },
		{ date: easterDateOffset(-2, easter), description: "Langfredag" },
		...
		{ date: new Date(yearString + "-05-17"), description: "Grunnlovsdag" },
		{
		...
		{ date: new Date(yearString + "-12-31"), description: "Nyttårsaften" },
  	];
